...
Examples:
  ...
  php artisan remote:cmds production fresh logs    # Pre-select production, fresh and logs with confirmation';
     ...
     * Note: PHP path will be dynamically replaced based on environment
     */
    ...
    public function handle(): int
    {
        ...
        if (! empty($selectedCommands)) {
            ...
            if ($this->confirm('Execute these commands now via SSH?', false)) {
                ...
                $this->newLine();
                $this->executeCommands($sshUser, $sshHost, $sshPort, $remotePath, $selectedCommands, $env);
            }
        }
    ...
    /**
     * Prepare command by replacing PHP_PATH and COMPOSER_PATH placeholders
     * Uses PHP 8.5 for Hostinger environments (both production and staging)
     */
    private function prepareCommand(string $command, string $env): string
    {
        // Both Hostinger production and staging require PHP 8.5 and composer2
        if ($env === 'production' || $env === 'staging') {
            $phpPath = '/opt/alt/php85/usr/bin/php';
            $composerPath = '/opt/alt/php85/usr/bin/php /usr/local/bin/composer2';
        } else {
            // Other environments use default paths
            $phpPath = 'php';
            $composerPath = 'composer';
        }
        
        $command = str_replace('PHP_PATH', $phpPath, $command);
        $command = str_replace('COMPOSER_PATH', $composerPath, $command);
     ...
     */
    private function executeCommands(string $user, string $host, int $port, string $remotePath, array $selectedCommands, string $env): void
    {
        ...
        foreach ($selectedCommands as $cmd) {
            ...
            $command = $this->prepareCommand(self::ALLOWED_COMMANDS[$cmd], $env);
